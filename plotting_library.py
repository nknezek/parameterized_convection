...
def plot_thermal_history2(times, T_cmb, T_upper_mantle, times_calculated=None, T_upper_magma_ocean=None, D_magma_ocean=None, r_i=None, T_i=None, D_st=None):
    fig = plt.figure(figsize=(8,6))
    plt.plot( times, T_cmb)
    # plt.plot( times_calculated, T_upper_magma_ocean)
    plt.plot( times, T_upper_mantle)
    ...
    plt.plot(times, T_i)
    plt.plot( times, r_i/1e3)
    plt.plot(times[:-1], D_st/1e3)
    plt.title("Thermal Evolution of Earth")
    plt.ylabel(r"Temperature (K) or radius (km)")
    plt.xlabel(r"Time (Myr)")
    plt.legend(["T - CMB", "T - upper mantle", "T - ICB", "Inner-core radius", 'Stable layer thickness' ], loc=0)
    plt.grid()
